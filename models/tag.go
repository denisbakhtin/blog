...
func GetTags() ([]Tag, error) {
	var list []Tag
	err := db.Select(&list, "SELECT tags.name, count(poststags.tag_name) as post_count FROM tags LEFT OUTER JOIN poststags ON tags.name=poststags.tag_name GROUP BY tags.name ORDER BY name ASC")
	return list, err
}
